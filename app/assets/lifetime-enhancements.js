(function () {
  ...
  (function showStartupSplash() {
    var MAX_MS = 7500; /* fallback safety cap (video is ~6.5s) in case video events never fire */
  ...
  var EAT_FROG_KEY      = "lt_eat_frog_v1";        /* { date: "YYYY-MM-DD", tasks: [{text, done}] } */
  ...
  var NUDGE_KEY         = "lt_waste_nudge_v1";      /* { lastNudge: timestamp } */
  ...
  function taskDateLabel(dateStr, completed, timeStr) {
    ...
    var label;
    if (d < now)                    label = "\u26A0 Missed";
    else if (d.getTime() === now.getTime()) label = "Today";
  ...
  function addStyle() {
    ...
    s.textContent = [
      ...
      ".lt-task-item{display:flex;align-items:center;gap:12px;padding:13px 16px;border-bottom:1px solid hsl(var(--border));-webkit-tap-highlight-color:transparent}",
      ...
      ".lt-tasks-sheet-main::placeholder{color:hsl(var(--muted-foreground))}",
      ...
      ".lt-tasks-sheet-date{flex:1;border:none;background:transparent;color:hsl(var(--foreground));font:inherit;font-size:14px;outline:none}",
      ".lt-tasks-sheet-notes{font-size:14px;width:100%;border:none;background:transparent;color:hsl(var(--foreground));font-family:inherit;padding:10px 0;border-top:1px solid hsl(var(--border));outline:none;caret-color:hsl(var(--primary))}",
      ".lt-tasks-sheet-notes::placeholder{color:hsl(var(--muted-foreground))}",
      ".lt-tasks-sheet-submit{margin-top:14px;width:100%;background:hsl(var(--primary));color:white;border:none;border-radius:12px;padding:13px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;-webkit-tap-highlight-color:transparent}",
      ".lt-tasks-sheet-submit:active{opacity:.85}",
  ...
  function addStyle3() {
    ...
    s.textContent = [
      ...
      "#lt-top-status-bar.lt-ts-running .lt-ts-dot{background:#ff3b30;animation:lt-pulse 1.2s infinite}",
      ...
      ".lt-curr-option .lt-curr-flag{font-size:24px;width:36px;text-align:center}",
  ...
  function sectionFor(task) {
    ...
    var now = new Date(); now.setHours(0,0,0,0);
    var tom = new Date(now); tom.setDate(tom.getDate() + 1);
    var d   = new Date(task.date + "T00:00:00");
  ...
  function makeTaskHTML(t) {
    ...
    var overdue = !t.completed && isOverdueDate(t.date);
    ...
    activeOverlay.innerHTML = (
      ...
      '<div class="lt-tasks-sheet" id="lt-tasks-sheet">' +
        ...
        '<div class="lt-tasks-sheet-row">' +
          '<span class="lt-tasks-sheet-row-label">Date</span>' +
          '<input id="lt-task-date" class="lt-tasks-sheet-date" type="date" value="' + today() + '">' +
        '</div>' +
        '<div class="lt-tasks-sheet-row">' +
          '<span class="lt-tasks-sheet-row-label">Time</span>' +
          '<input id="lt-task-time" class="lt-tasks-sheet-date" type="time">' +
        '</div>' +
        '<div class="lt-tasks-sheet-row" style="border-top:none;padding-top:0">' +
          '<span class="lt-tasks-sheet-row-label">Notes</span>' +
  ...
  function upsertRunningBanner() {
    ...
    /* The app only marks the page "lt-authed" once the user is actually
       ...
       no activity data to show a status for at all. */
    if (!document.body.classList.contains("lt-authed")) {
    ...
    if (!bar) {
      ...
      document.getElementById("lt-ts-stop-btn").addEventListener("click", function () {
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     FEATURE 2 — Smart Nudge: fires after 30min on a Waste category
  ══════════════════════════════════════════════════════════════════════════ */

  var NUDGE_THRESHOLD_MS = 30 * 60 * 1000; /* 30 minutes */

  function checkWasteNudge() {
    var pending = readJson(PENDING_TIMER_CAT_KEY, null);
    if (!pending || !pending.category) return;
    if (WASTE_CATEGORIES.indexOf(pending.category) === -1) return;

    var startedAt = Number(readJson(RUNNING_TIMER_KEY, 0));
    if (!startedAt) return;
    var elapsed = Date.now() - startedAt;
    if (elapsed < NUDGE_THRESHOLD_MS) return;

    /* Don't nudge more than once per session */
    var nudgeData = readJson(NUDGE_KEY, {});
    if (nudgeData.lastNudge && (Date.now() - nudgeData.lastNudge) < NUDGE_THRESHOLD_MS) return;
    writeJson(NUDGE_KEY, { lastNudge: Date.now() });

    addStyle3();
    var tv = readJson(TIMEVALUE_KEY, null);
    var wasted = tv ? money(Number(tv.perMinute) * 30) : (getCurrency().symbol + "150");
    ...
    toast.innerHTML =
      ...
      '<div class="lt-nudge-body">' +
        '<p>Friend, time is slipping away!</p>' +
        '<p>' + wasted + ' worth of time has already been wasted \u2014 let\u2019s get back to progress?</p>' +
      '</div>';
  ...
  function injectAccountCard() {
    ...
    var main = document.querySelector("main, [class*='settings'], [class*='content']");
    if (!main) return;
    addStyle3();
    ...
    card.innerHTML =
      '<div style="padding:16px 20px;border-bottom:1px solid hsl(220 13% 92%)">' +
        ...
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px">' +
          '<div style="min-width:0">' +
            '<p style="color:hsl(230 40% 16%);font-size:14px;font-weight:700;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + (user && user.email ? escapeHtml(user.email) : "Signed in") + '</p>' +
            '<p style="color:hsl(220 10% 55%);font-size:11px;margin:2px 0 0">Your data stays on this device</p>' +
          '</div>' +
    ...
    goalOptions.forEach(function (opt) {
      ...
      b.addEventListener("click", function () {
        ...
        card.remove();
        setTimeout(injectAccountCard, 0);
      });
  ...
  function refreshPlanGatedUI() {
    ...
    var gatedWrap = document.getElementById("lt-telegram-gate-wrap");
    if (gatedWrap) {
      /* unwrap: move children back out, then remove the wrapper */
      while (gatedWrap.firstChild) gatedWrap.parentElement.insertBefore(gatedWrap.firstChild, gatedWrap);
      gatedWrap.remove();
    }
    document.querySelectorAll("[data-lt-tile-injected]").forEach(function (el) { el.remove(); });
    setTimeout(function () { injectAccountCard(); gateTelegramSettings(); }, 30);
  }
  ...
  function gateTelegramSettings() {
    if (isPro()) {
      var existingWrap = document.getElementById("lt-telegram-gate-wrap");
      if (existingWrap) {
        while (existingWrap.firstChild) existingWrap.parentElement.insertBefore(existingWrap.firstChild, existingWrap);
        existingWrap.remove();
      }
      return;
    }
    if (document.getElementById("lt-telegram-gate-wrap")) return; /* already wrapped */

    var headers = document.querySelectorAll("h2,h3,p");
    for (var i = 0; i < headers.length; i++) {
      if (!/telegram daily reports/i.test(headers[i].textContent || "")) continue;
      var headerWrap = headers[i].closest("div") || headers[i];
      var settingsBlock = headerWrap.nextElementSibling;
      var parent = headerWrap.parentElement;
      if (!parent) return;

      var wrap = document.createElement("div");
      wrap.id = "lt-telegram-gate-wrap";
      wrap.style.cssText = "position:relative;";
      parent.insertBefore(wrap, headerWrap);
      wrap.appendChild(headerWrap);
      if (settingsBlock) wrap.appendChild(settingsBlock);

      var dim = document.createElement("div");
      dim.style.cssText = "opacity:.4;pointer-events:none;filter:grayscale(.3);";
      /* move the real content into the dim wrapper so it visually greys out, same as the Budget Tracker tile */
      while (wrap.firstChild) dim.appendChild(wrap.firstChild);
      wrap.appendChild(dim);

      var badge = document.createElement("div");
      badge.style.cssText = "position:absolute;top:10px;right:14px;background:hsl(230 40% 16%);color:#fff;font-size:10px;font-weight:800;letter-spacing:.05em;padding:4px 9px;display:flex;align-items:center;gap:4px;z-index:2;pointer-events:none;";
      badge.innerHTML = "\uD83D\uDD12 PRO";
      wrap.appendChild(badge);

      var overlay = document.createElement("div");
      overlay.style.cssText = "position:absolute;inset:0;z-index:3;cursor:pointer;background:transparent;";
      overlay.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
  ...
  function runEnhancements() {
    ...
    enforceActivityGraceIfNeeded();
    updateActivityLimitBadge();
    normalizeOriginalLabels();
    hijackDateInputs();
    ensureActivityActionDescriptions();
    updateSavedTimeValueCard();
    mountLifeHubTools();
    pollRunningTimer();
    injectJournalFullView();
    upsertRunningBanner();
    checkWasteNudge();
    updateStreak();
    buildLifeProgressCard();
    buildEatTheFrogCard();
    restyleTopNav();
    injectCurrencyChipsIntoOverlays();
    replaceRupeeGlobally();
  ...
  function showNavMask() {
    ...
    _navMaskEl.style.cssText =
      "position:fixed;inset:0;z-index:2147483000;background:hsl(var(--background));" +
      "pointer-events:none;";
